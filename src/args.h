...
"  -F url         : url of pool or node to mine on, if not specified, will solo mine to http://127.0.0.1:8543\n"
"  -t nThreads    : number of threads to use (if not specified will use maximum logical threads available)\n"
"  -n node_url    : optional node url, to print more stats (pool mining only)\n"
"  -r rate        : pool refresh rate, ex: 3s, 2.5m, 1300ms, default is 3s\n"
"  --solo         : solo mining, -F needs to be the node url such as http://127.0.0.1:8543\n"
"  --proxy        : proxy to use, ex: --proxy socks5://127.0.0.1:9150"
"  --argon x,y,z  : testing: use specific argon params (ex: 4,512,1)\n"
